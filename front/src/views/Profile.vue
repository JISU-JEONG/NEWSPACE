<template>
  <div>
    <div class="header_box">
      ...
      <div
        class="header_email"
        v-if="emailstatus !== 'true'"
      >이메일 인증을 완료하여 관심키워드 뉴스를 받아보세요</div>
      <div class="header_email">
        <v-btn text v-if="emailstatus !== 'true'" @click="emailsend()">이메일 인증하기</v-btn>
        <span v-else class="d-flex align-center justify-center">
          <v-icon color="green">mdi-check-decagram</v-icon>이메일 인증 완료
        </span>
        <v-snackbar
          v-model="snackbar"
          bottom
    ...
    <v-container class="px-1" style="min-height:850px">
      <v-tabs :vertical="is_size">
        ...
        <v-tab-item class="px-2">
          <v-container fluid>
            <div style="min-height:800px">
              ...
              <v-row v-if="like_list.length !== 0">
                <v-col
                  ...
                  lg="3"
                >
                  <v-card height="350px" class="cardhover">
                    <v-img :src="like_list[page-1][i-1].thumb" height="150px"></v-img>

                    <v-card-title>
                      <h4 v-line-clamp="2">{{like_list[page-1][i-1].title}}</h4>
                    </v-card-title>

                    <v-card-subtitle>
                      <p v-line-clamp="5">{{like_list[page-1][i-1].bodytext}}</p>
                    </v-card-subtitle>
                  </v-card>
                </v-col>
              </v-row>
            ...
            <div class="text-center">
              <v-pagination v-model="page" :length="pagetotal" circle total-visible="6"></v-pagination>
            </div>
          </v-container>
        </v-tab-item>
        <v-tab-item class="px-2">
          <v-container fluid>
            <div style="min-height:800px">
              ...
              <v-row v-if="recent.length !== 0">
                ...
                >
                  <v-card height="350px" class="cardhover">
                    <v-img :src="recent[i-1].thumb" height="150px"></v-img>

                    <v-card-title>
                      <h4 v-line-clamp="2">{{recent[i-1].title}}</h4>
                    </v-card-title>

                    <v-card-subtitle>
                      <p v-line-clamp="5">{{recent[i-1].bodytext}}</p>
                    </v-card-subtitle>
                  </v-card>
                </v-col>
              </v-row>
        ...
        <v-tab-item>
          <v-container>
            <div class="likeheader">
              <strong>{{user.member.name}}님 관심 키워드</strong>
              <v-btn text class="updatedfont" @click.stop="dialog = true">수정</v-btn>
              <hr />
            </div>
            <Treechart :keyword="selectedKeywords" />
...
export default {
  ...
  data() {
    return {
      ...
      error: false,
      errorMessages: "",
      isemailcheck: this.$store.state.certifiedkey,
      page: 1,
      like_list: [],
      pagetotal: 0,
      recent: []
  ...
  computed: {
    ...
    emailstatus(){
      return this.$store.state.certifiedkey;
    }
  }
};
</script>
